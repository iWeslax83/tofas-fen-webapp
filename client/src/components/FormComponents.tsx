import React, { useState, useEffect, forwardRef, memo, useImperativeHandle } from 'react';
import {
  Eye,
  EyeOff,
  Check,
...
// F-M9: wrap in React.memo so bulk-import forms with dozens of FormField
// instances don't re-render every single field on every keystroke in any
// other field. Props are largely primitive/string, so default shallow
// equality is correct.
export const FormField = memo(
  forwardRef<FormFieldRef, FormFieldProps>(
    (
      {
        id,
        name,
      ...
      const validateField = async (
        value: string,
      ): Promise<{ isValid: boolean; error: string; success: string }> => {
      ...
      return (
        <div className={getFieldClasses()}>
          ...
          <div className="form-field__input-container">
            {/* Icon */}
            {icon && <div className="form-field__icon form-field__icon--left">{icon}</div>}
            ...
            {/* Default Icon */}
            {!icon && (
              <div className="form-field__icon form-field__icon--left">{getDefaultIcon()}</div>
            )}
...
export const Form: React.FC<FormProps> = ({
  ...
  return (
    <form
      ...
      noValidate
    >
      <div className="enhanced-form__content">{children}</div>
...
export const FormFieldGroup: React.FC<FormFieldGroupProps> = ({
  ...
  return (
    <div className={`form-field-group form-field-group--${columns}-col ${className}`}>
      ...
      <div className="form-field-group__content">{children}</div>
    </div>
  );
};
  ...
  return (
    <form onSubmit={handleSubmit} className={`search-field ${className}`}>
      <div className="search-field__container">
        ...
        />
        {loading && <div className="search-field__spinner"></div>}
        {value && (
          <button
            type="button"
            onClick={() => onChange('')}
