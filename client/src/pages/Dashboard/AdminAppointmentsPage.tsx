...
export default function AdminAppointmentsPage() {
  ...
  const updateStatus = async (id: string, status: string, reason?: string) => {
    ...
    try {
      ...
      toast.success(`Randevu ${statusMsg[status] || 'güncellendi'}`);
      setRejectingId(null);
      setRejectionReason('');
      await fetchData();
    } catch {
  ...
  return (
    <ModernDashboardLayout pageTitle="Randevu Başvuruları">
      <div style={{ padding: '24px', maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
          <CalendarDays size={28} />
          <h1 style={{ margin: 0, fontSize: 24 }}>Randevu Başvuruları</h1>
        </div>
                ...
                >
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', fontSize: 14 }}>
                      <span
                        style={{ display: 'flex', alignItems: 'center', gap: 4, color: '#374151' }}
                      >
                    ...
                    <div style={{ marginTop: 8, fontSize: 14, color: '#374151' }}>
                      <strong>Amaç:</strong> {apt.purpose}
                    </div>
                    {apt.notes && (
                  ...
                  >
                    <span
                      style={{
                        padding: '4px 10px',
                        borderRadius: 6,
                        background: `${statusColors[apt.status]}15`,
                    ...
                    {apt.status === 'pending' && rejectingId !== apt._id && (
                      <div style={{ display: 'flex', gap: 6 }}>
                        ...
                        <button
                          onClick={() => setRejectingId(apt._id)}
                          disabled={updating === apt._id}
                          style={{
                            padding: '6px 12px',
                            borderRadius: 6,
                    ...
                    {rejectingId === apt._id && (
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', gap: 6 }}>
                          <button
                            onClick={() => updateStatus(apt._id, 'rejected', rejectionReason)}
                            disabled={updating === apt._id}
                            style={{
                              flex: 1,
                              padding: '6px 12px',
