...
// F-C2 fix: the Zustand `persist` middleware previously wrote the full user
// object (id, role, classroom, etc.) to localStorage. An XSS would exfiltrate
// it in a single read, and logout couldn't reliably clear the ghost. We now
// keep auth state in memory only and bootstrap it via `/api/auth/me` on app
// mount (see `checkAuth`). HttpOnly cookies still carry the real credentials.
//
// One-shot migration: wipe any ghost data left by older builds that still
// had the persist middleware. This runs at module load (once per tab).
...
export const useAuthStore = create<AuthStore>()((set, get) => ({
  ...
  logout: async () => {
    // Legacy localStorage cleanup — strip keys written by prior versions
    // of this store (before F-C2). These should no longer exist on fresh
    // sessions, but clearing them on logout defends against ghost state
    // from a previous build that still had the persist middleware.
    TokenManager.clearTokens();
    try {
      localStorage.removeItem('user');
      localStorage.removeItem('csrf_token');
      localStorage.removeItem('auth-storage'); // old zustand persist key
    } catch {
      /* storage disabled / quota / safari private mode */
    }
