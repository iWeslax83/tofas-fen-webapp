  ...
  try {
    ...
    if (!conversation) {
      ...
      const participants = [
        ...
        ...adminIds.map(id => ({ userId: id, role: 'admin', joinedAt: new Date(), isActive: true }))
      ];

      conversation = new Conversation({
        id: generateId(),
        type: 'group',
        title: `Ziyaretci Sohbet - ${user?.adSoyad || authUser.userId}`,
        description: 'Yeni kayit ziyaretci ile yonetici iletisim kanali',
        participants,
        admins: adminIds,
        settings: {
    ...
    logger.error('Visitor chat conversation error', { error: error instanceof Error ? error.message : error });
...
router.post('/messages', authenticateJWT, authorizeRoles(['ziyaretci', 'admin']), async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Visitor chat send message error', { error: error instanceof Error ? error.message : error });
    res.status(500).json({ error: 'Mesaj gonderilirken hata olustu' });
  }
});

// Admin: Get all visitor conversations
router.get('/conversations', authenticateJWT, authorizeRoles(['admin']), async (_req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ error: 'Sohbetler alinirken hata olustu' });
  }
});
