...
router.get("/student/:studentId", authenticateJWT, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Öğrenci ödevleri getirilirken hata oluştu' });
  }
});
...
router.get("/teacher/:teacherId", authenticateJWT, async (req, res) => {
  try {
    ...
    console.error('Teacher homework fetch error:', error);
    res.status(500).json({ error: 'Öğretmen ödevleri getirilirken hata oluştu' });
  }
});
