...
router.post('/subscribe', authenticateJWT, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error saving push subscription', { error: error instanceof Error ? error.message : error });
    res.status(500).json({ error: 'Push subscription kaydedilemedi' });
  }
});

// DELETE /unsubscribe — Push subscription sil
router.delete('/unsubscribe', authenticateJWT, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error removing push subscription', { error: error instanceof Error ? error.message : error });
