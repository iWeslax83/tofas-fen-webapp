...
router.get("/user/:userId", authenticateJWT, async (req, res) => {
  try {
    ...
    logger.error('Talep getirme hatasi', { error: error instanceof Error ? error.message : error });
...
router.get("/", authenticateJWT, authorizeRoles(['admin', 'teacher']), async (req, res) => {
  try {
    ...
    logger.error('Talep listeleme hatasi', { error: error instanceof Error ? error.message : error });
    res.status(500).json({ error: "Sunucu hatası" });
  }
});
...
router.post("/", authenticateJWT, validateRequestData, async (req, res) => {
  try {
    ...
    logger.error('Talep olusturma hatasi', { error: error instanceof Error ? error.message : error });
    res.status(500).json({ error: "Sunucu hatası" });
  }
});
...
router.patch("/:id", authenticateJWT, authorizeRoles(['admin', 'teacher']), async (req, res) => {
  try {
    const updated = await RequestModel.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
    if (!updated) {
      return res.status(404).json({ error: "Talep bulunamadı" });
    }
    res.json(updated);
  } catch (error) {
    logger.error('Talep guncelleme hatasi', { error: error instanceof Error ? error.message : error });
    res.status(500).json({ error: "Sunucu hatası" });
  }
});
