...
router.get('/', authenticateJWT, async (req, res) => {
  try {
    ...
    const filter: FilterQuery<ISupervisorList> = {};
    ...
    logger.error('Belletmen listesi getirme hatasi', {
...
router.get('/:id', authenticateJWT, async (req, res) => {
  try {
    ...
    logger.error('Belletmen nobeti getirme hatasi', {
      error: error instanceof Error ? error.message : error,
    });
    res.status(500).json({ error: 'Sunucu hatası' });
  }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      logger.error('Belletmen nobeti olusturma hatasi', {
        error: error instanceof Error ? error.message : error,
      });
      res.status(500).json({ error: 'Sunucu hatası' });
    }
...
router.put(
  ...
  async (req, res) => {
    try {
      const supervisor = await SupervisorList.findByIdAndUpdate(req.params.id, req.body, {
        new: true,
        runValidators: true,
      });
      if (!supervisor) {
      ...
      logger.error('Belletmen nobeti guncelleme hatasi', {
        error: error instanceof Error ? error.message : error,
      });
      res.status(500).json({ error: 'Sunucu hatası' });
    }
...
router.delete('/:id', authenticateJWT, authorizeRoles(['admin']), async (req, res) => {
  try {
    ...
    logger.error('Belletmen nobeti silme hatasi', {
      error: error instanceof Error ? error.message : error,
    });
    res.status(500).json({ error: 'Sunucu hatası' });
  }
