...
router.post(
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      logger.error('Error cleaning memory', {
        error: error instanceof Error ? error.message : error,
      });
      res.status(500).json({
        success: false,
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    try {
      const [systemMetrics, databaseMetrics, apiMetrics, criticalMetrics, recentOptimizations] =
        await Promise.all([
