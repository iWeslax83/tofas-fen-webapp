...
const getRefreshSecret = () => process.env.JWT_REFRESH_SECRET || 'test-refresh-secret-key-for-testing-only'
...
describe('JWT Utils', () => {
  ...
  describe('authenticateJWT', () => {
    ...
    it('should return 401 for invalid token', async () => {
      ...
      expect(res.status).toHaveBeenCalledWith(401)
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ error: 'Invalid or expired access token' }))
    })
  })
