...
describe('Security Tests', () => {
  ...
  describe('Authentication Security', () => {
    it('should not reveal user existence in login', async () => {
      ...
      const existingResponse = await request(app).post('/api/auth/login').send({
        id: existingUser,
        sifre: 'wrongpassword',
      });

      const nonExistingResponse = await request(app).post('/api/auth/login').send({
        id: nonExistingUser,
        sifre: 'wrongpassword',
      });
  ...
  describe('Content Security', () => {
    it('should validate JSON payloads', async () => {
      const invalidPayloads = ['invalid json', '{invalid:json}'];
